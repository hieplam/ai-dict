...
> 1. Root devDependencies include **`size-limit`** and **`@size-limit/file`** (so `pnpm size` resolves). *Not in Bundle 01's current Step 5 install list — that list must be extended, exactly as it already installs `eslint`/`vitest`/`prettier` for the other frozen scripts.*
