...
- Start in a fresh git worktree under `.claude/worktrees/` on branch `feat/a7-pin-cards`.
- Commit subject convention for every task in this plan:
  `feat: <imperative summary> (A7)`. No `Co-Authored-By` trailer, no attribution
  footer.
...
- PR: title `feat: pin cards (A7)`; body carries a written **"Testing performed"** section
...
git commit -m "feat: pin control UI on lookup-card, ICON_PIN token (A7)"
```
...
git commit -m "feat: floating-pin non-modal draggable host + registration (A7)"
```
...
git commit -m "feat: pinCurrent() detach orchestration in inline-bottom-sheet-renderer (A7)"
```
...
git commit -m "feat: e2e coverage for pinning, drag, close, cap, and page-nav teardown (A7)"
```
...
Title: `feat: pin cards (A7)`. Regular merge (no squash). No `## JIRA ticket` section
(`docs/git-conventions.md` — this repo has no ticket tracker). Include a **"Testing performed"** section per this worktree's
