...
Shadow DOM specifically added to block other extensions' scripts from reaching it. This mechanism —
and the resulting fact that no selectable PDF text is reachable from the tab's top-level document —
is grounded in Chromium's own architecture documentation (Sources); this spike's scripted probe
could **not** reproduce it directly, because the Playwright-bundled test Chromium ships no
PDF-viewer component and so never renders the viewer to observe (see Probe 1, "Precise root
cause"). What the probe DID confirm — directly and reproducibly, against this repo's real built
extension — is the one fact a `manifest.json`/permissions change might otherwise be blamed on: our
content scripts DO run on a PDF tab's top-level document (`contentScriptRan: true`), so the blocker
is not that our scripts fail to execute. No `manifest.json` change can cross the isolation boundary
the Chromium docs describe — it applies uniformly to every extension, not just this one, and no
additional host permission narrows it.
...
> **On candidate count (design spec §2 D4):** D4 pins three candidates — a custom `pdf.js` viewer,
> a DevTools/`chrome.debugger` overlay, and "not worth it." The table presents the `pdf.js` viewer
> in two entry-point variants (blanket redirect vs. opt-in "Open in AI Dictionary") because their
> permission and risk profiles differ materially and the recommendation below turns on that
> difference — both are the same underlying candidate, not a distinct fourth approach.
