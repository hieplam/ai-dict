...
chrome.runtime.onMessage.addListener((msg: { to?: string; state?: string; payload?: unknown }, sender) => {
  ...
  if (msg.state === 'loading') set({ kind: 'loading' });
    ...
    if (!isLookupResult(msg.payload)) { console.warn('[side-panel] invalid result payload'); return; }
    const r = msg.payload;
