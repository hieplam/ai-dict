---
id: c3-117
c3-seal: d2935e442c8c8694759e60a69032de8b3569f9d40975daf913c3b8ce25c0818a
title: ui-components
type: component
category: feature
parent: c3-1
...
This component owns all user-visible elements used by the ai-dict extension. It exposes four custom elements: `<lookup-trigger>` renders a floating "Define" button in open shadow DOM with `z-index: 2147483647` (pinned explicitly because `all: initial` resets z-index to `auto`, causing the trigger to be occluded by page stacking contexts); `<lookup-card>` renders dictionary results in three states (loading, result, error) with content placed in the element's **light DOM** projected through a `<slot>` so that the content-script renderer can drive it across the Chrome MV3 isolated-world boundary (Chromium 390807); `<bottom-sheet>` provides an accessible modal overlay with ARIA `role="dialog"`, focus trapping (Tab / Shift+Tab), Escape dismiss, scrim-click dismiss, reduced-motion support, and focus restoration on disconnect; `<settings-form>` renders the full options form (AI-provider picker with one key row per provider, env-key lock for Gemini) in a trusted extension page only, collecting API key, target language, the card-format field, cache/history toggles, and an Advanced disclosure that edits the full prompt envelope (advanced override, #62). The Advanced textarea prefills the built-in `PROMPT_ENVELOPE` for editing but emits `''` until the user actually edits it. `<settings-form>` also hosts a Konami-gated ("Developer mode") panel: a window keydown listener matches ↑↑↓↓←→←→BA (ignoring events targeting an input/textarea/select via `composedPath()[0]`) and, on the full sequence, reveals a session-only `<pre id="devprompt">` that renders the exact assembled prompt via `buildPrompt` using `textContent` only (never innerHTML, per rule-sanitize-model-output). The `SafeHtml` branded type is exported from this component and is the type contract that forces all rendering code to obtain sanitized HTML through c3-116. This component does NOT perform any sanitization itself, does NOT communicate with the service worker, and does NOT own any business logic.
...
| Shared styling utility | adoptStyles(root: ShadowRoot, css: string) creates a CSSStyleSheet, calls replaceSync(css), and sets root.adoptedStyleSheets; the Paperlight tokens.ts exports selector-less BASE_VARS (--adp-* primitives + default sepia --ad-*) and a THEME_CSS override block, both folded into each component's single sheet so adoptedStyleSheets.length stays 1; see packages/app/src/ui/styles/adopt.ts, packages/app/src/ui/styles/tokens.ts | ref-web-components-shadow-dom |
| Hardened slotted setup CSS | The setup-invite ::slotted rules (.mark, .setup-title, .setup-text, .setup-cta) are declared !important: slotted nodes live in the host page's light DOM, where the outer tree's NORMAL declarations beat the shadow's normal ::slotted() ones (CSS Scoping tree-context order), so a host reset like button{margin:0} stripped the centering; inner-tree IMPORTANT declarations win that tiebreak regardless of outer specificity; see packages/app/src/ui/lookup-card.ts | ref-web-components-shadow-dom |
...
| Canonical UI icons (ICON_SETTINGS, ICON_CLOSE, ICON_SHIELD, ICON_TRASH) | OUT | CSP-safe inline-SVG strings pinned in styles/tokens.ts as the single source (§5.10), consumed by lookup-card, side-panel-view, settings-form, onboarding-view; stroke=currentColor, aria-hidden | Centralized so the glyph set can never drift again; ICON_SETTINGS is re-exported from lookup-card for back-compat | packages/app/src/ui/styles/tokens.ts — export const ICON_* |
...
| SettingsForm Theme control | IN | A segmented control (.seg, role=group) of aria-pressed buttons (sepia/dark/contrast/system); pressing one stamps data-ad-theme on the host for a live preview and the pressed segment is the Theme emitted on save (§5.8) | Live preview only re-themes the open form; persistence still happens on Save via the composition root | packages/app/src/ui/settings-form.ts — setThemePref/getThemePref |
| data-ad-theme host attribute | IN | The "Paperlight" theme is stamped as data-ad-theme on each themed host (lookup-trigger, lookup-card, bottom-sheet, side-panel-view, onboarding-view, settings-form). No attribute = sepia (warm-paper default, BASE_VARS on :host); data-ad-theme of dark or contrast re-binds the --ad-* semantic layer unconditionally, and system applies dark only under prefers-color-scheme: dark (THEME_CSS in tokens.ts). The settings-form is FULLY THEMED (§5.8): it folds in BASE_VARS + THEME_CSS so the attribute re-binds the whole --ad-* palette (color-scheme rides along), exactly like the card and side panel — no native browser-chrome surface remains. An ATTRIBUTE (not a JS property) so it crosses the MV3 MAIN/isolated world boundary and survives the trigger all:initial | Stamped by composition roots from stored settings (Theme = sepia/dark/contrast/system); legacy 'light' is coerced to 'sepia' on storage read | packages/app/test/ui/lookup-trigger.test.ts — theme contract test |
