...
- [ ] Chrome smoke: clean profile, set key, look up a word on Wikipedia, verify card + history + cache hit on repeat.
  - **Note:** Chrome lookup e2e runs automatically in CI (`e2e-chrome` job) via `xvfb-run -a`; no manual step needed — verify the CI job is green before tagging. If the job fails, check the virtual display setup in `.github/workflows/ci.yml` (`e2e-chrome` job).
